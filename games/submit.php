	...
	if($champ_test != 0)
	{
		if(($champ['score'] < $score && $game['score_type'] == "DESC") || ($champ['score'] > $score && $game['score_type'] == "ASC"))
		{
			...
			if($champ['uid'] != $update_champ['uid'])
			{
				...
				$message = $lang->sprintf($lang->email_champbeaten, $ex_champ['username'], $mybb->user['username'], $game['title'], $champ['score'], $score, $mybb->settings['bbname'], $mybb->settings['bburl'], "games.php?action=play&gid=" . $gid);
